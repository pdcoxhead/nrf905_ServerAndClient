// nrf905_server.pde
// -*- mode: C++ -*-
// Example sketch showing how to create a simple messageing server
// with the RH_NRF905 class. RH_NRF905 class does not provide for addressing or
// reliability, so you should only use RH_NRF905  if you do not need the higher
// level messaging abilities.
// It is designed to work with the other example nrf905_client
// Tested on Teensy3.1 with nRF905 module
// Tested on Arduino Due with nRF905 module (Caution: use the SPI headers for connecting)
#include <stdio.h>
#include <SPI.h>
#include <RH_NRF905.h>

// Singleton instance of the radio driver
//RH_NRF905 nrf905(7, 9, 10);
RH_NRF905 nrf905(7, 9, 10);

void handleMessage(const char* message);

void setup()
{
  Serial.begin(9600);
  while (!Serial)
    ; // wait for serial port to connect. Needed for Leonardo only
  if (!nrf905.init())
  {
    Serial.println("\ninit failed\n");
  }
  else
  {
    Serial.println("\ninit succeeded\n");
  }

    pinMode(8, OUTPUT);
    digitalWrite(8, HIGH);
  nrf905.setModeIdle();
  // Defaults after init are 433.2 MHz (channel 108), -10dBm
}

void loop()
{
  static int counter = 0;
  nrf905.setModeRx();
  int8_t status = nrf905.statusRead();
  Serial.print("status = ");
  Serial.print(status);
  Serial.print("\n");
  if (nrf905.available())
  {
    // Should be a message for us now
    uint8_t buf[RH_NRF905_MAX_MESSAGE_LEN];
    uint8_t len = sizeof(buf);
    if (nrf905.recv(buf, &len))
    {
      Serial.print("got request: ");
      Serial.println((char*)buf);

      handleMessage((const char* )buf);

    }
    else
    {
      Serial.println("recv failed");
    }
  }
  else
  {
    if (++counter % 100 == 0)
    {
      Serial.println("Not available");
      counter = 0;
    }
  }
}

typedef struct _MessageHandler {
  const char * message;
  void (*handler)(const char* data);
} MessageHandler;

void openGate(const char* data)
{
  nrf905.setModeTx();
  if (nrf905.available())
  {
    const uint8_t bGateButton[] = "";
    nrf905.send(&bGateButton[0], sizeof(bGateButton));
    nrf905.waitPacketSent();
  }
}

void pingBack(const char* data)
{
  nrf905.setModeTx();
  if (nrf905.available())
  {
    const uint8_t bPing[] = "PING";
    nrf905.send(&bPing[0], sizeof(bPing));
    nrf905.waitPacketSent();
  }
}

MessageHandler handlers[] = {
  { "",     openGate },
  { "PING", pingBack }
};

void handleMessage(const char* message)
{
  if (message != NULL)
  {
    const int handlerCount = sizeof(handlers) / sizeof(handlers[0]);
    const int messageLength = strlen(message);
    int i;
    for (i = 0; i < handlerCount; i++)
    {
      const int handlerMsgLength = strlen(handlers[i].message);
      if (strncmp(handlers[i].message, message, handlerMsgLength) == 0)
      {
        handlers[i].handler((const char*) &message[handlerMsgLength]);
        Serial.println("Handled '%s'");
        break;
      }
    }
    if (i == handlerCount)
    {
      char sMessage[32];
      snprintf(sMessage, sizeof(sMessage) - 1, "No handler for '%s'", message);
      Serial.println(sMessage);
    }
  }
}
